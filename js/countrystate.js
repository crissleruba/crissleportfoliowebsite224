...
const city_names = {
    "BDS": ["Ashkāsham","Fayzabad","Jurm","Khandūd","Rāghistān","Wākhān"]
};
...
const createStatesNamesDropdown = () => {
    ...
    if (!state_names) {
        id_state_option.innerHTML = '<option>_</option>';
        return;
    }
    let option = '';
...
const createCitiesNamesDropdown = () => {
    let selected_state_code = id_state_option.value;
    // get city names
    let city_names = city_array[selected_state_code];

    // if invalid country code
    if (!city_names) {
        id_city_option.innerHTML = '<option>_</option>';
        return;
    }
    let option = '';
    option += '<option value="">Select City</option>';
    for (let i = 0; i < city_names.length; i++) {
        option += '<option value="' + city_names[i].code + '">' + city_names[i].name + '</option>';
    }
...
id_state_option.addEventListener('change', createCitiesNamesDropdown);

createCountryNamesDropdown();
createStatesNamesDropdown();
createCitiesNamesDropdown();
